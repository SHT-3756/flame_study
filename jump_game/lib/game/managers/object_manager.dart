...
import 'package:jump_game/game/model/dificulty_model.dart';
...
class ObjectManager extends Component with HasGameRef<DoodleDash> {
  ...
  double _generateNextY() {
    ...
    _rand.nextInt((maxVerticalDistanceToNextPlatform - minVerticalDistanceToNextPlatform).floor()).toDouble();
  ...
  Platform _semiRandomPlatform(Vector2 position) {
    if (specialPlatforms['spring'] == true &&
      ...
      return SpringBord(position: position);
    }
  ...
  void configure(int nextLevel, DifficultyModel config) {
